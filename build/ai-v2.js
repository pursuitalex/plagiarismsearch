...
${eyebrow('orange-500', 'Interpretation')}
      ...
      <!-- Recomposed 2026-08-26 at Olex's request: the three steps run down the left and
           the correction stands beside them at full height.

           It reads better this way than as a 3-across row above a full-width band. The
           steps are a sequence, and a vertical list is what a sequence looks like; each
           card is now a wide short row, so the number sits beside the text rather than
           stacked on top of it, which is what that shape wants.

           The right panel had room to spare once it was half-width and full-height, so
           it carries the diagram the sentence asks for: the same document twice, once
           with a single figure over the whole of it (AI Probability) and once with the
           flagged runs marked and no figure at all (Total AI Rate). That IS the
           correction — one is a number about everything, the other is which parts.
           70% is the sentence's own number; no second number is invented, and the marks
           are partial-width for the same reason. -->
      <div class="grid lg:grid-cols-2 gap-4 sm:gap-5 lg:gap-6 mb-4 lg:mb-5">

        <div class="rv-kids min-w-0 flex flex-col gap-4 sm:gap-5">
${COPY.s4.steps.map(([label, action], i) => `          <div class="flex-1 flex items-center gap-4 sm:gap-5 rounded-3xl sm:rounded-[28px] bg-white ring-1 ring-black/5 shadow-diffuse p-5 sm:p-6 lg:p-7">
            <span class="shrink-0 inline-flex w-10 h-10 rounded-full bg-ink-900 text-white text-[13px] font-bold items-center justify-center tabular-nums">${i + 1}</span>
            <div class="min-w-0">
              <h3 class="text-[15px] sm:text-[16px] lg:text-[16.5px] font-bold tracking-tight">${label}</h3>
              <p class="mt-1 text-[13px] sm:text-[13.5px] leading-relaxed text-ink-600">${action}</p>
            </div>
          </div>`).join('\n')}
        </div>
        ...
        <div class="rv min-w-0 rounded-3xl sm:rounded-4xl bg-ink-950 p-6 sm:p-7 lg:p-8 flex flex-col">
          ...
          <p class="text-[15.5px] sm:text-[16.5px] lg:text-[17.5px] font-bold tracking-tight leading-[1.42] text-white">${
          ...
          <div class="mt-auto pt-6 lg:pt-7">
            <div class="grid grid-cols-2 gap-3 sm:gap-3.5" aria-hidden="true">

              <!-- one figure, and it is about the whole thing -->
              ...
              <!-- which parts, and no figure at all -->
              <div class="min-w-0 rounded-2xl bg-white/[.05] ring-1 ring-white/10 p-3.5 sm:p-4">
